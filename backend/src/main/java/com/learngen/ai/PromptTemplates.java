...
public final class PromptTemplates {
    ...
    public static final String PATH_AGENT_SYSTEM = """
            ...
            ## 任务
            根据学生画像、学习历史和当前进度，输出个性化的机器学习学习路径。
            ...
            ## 输出格式要求（重要）
            1. **必须输出严格的 JSON 格式**，不要添加解释、Markdown 代码块标记或多余文字
            2. JSON 结构如下：
               {
                 "steps": [
                   {
                     ...
                     "description": "步骤描述",
                     "resourceType": "课程讲解/思维导图/练习题库/拓展阅读/代码实操",
                     "estimatedMinutes": 60,
                     "completed": false
                   }
                 ]
            ...
            3. 步骤数 5-8 个，由浅入深
            4. index 从 0 开始递增
            5. **必须跳过已完成的步骤**，不要重复安排相同的学习内容
            6. 结合知识点依赖图（基础概念→经典算法→无监督→深度学习→实战）
            7. 根据画像调整：基础差→多基础；基础好→快进到高级
            8. 所有字段必须有值，不能为 null
            """;
